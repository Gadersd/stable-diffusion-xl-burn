...
def save_resnet_block(resnet_block, path):
    ...
    if hasattr(resnet_block, 'nin_shortcut'):
...
def save_decoder_block(decoder_block, path):
    ...
    if hasattr(decoder_block, 'block'):
    ...
    if hasattr(decoder_block, 'upsample'):
...
def save_encoder_block(encoder_block, path):
    ...
    if hasattr(encoder_block, 'block'):
    ...
    if hasattr(encoder_block, 'downsample'):
        conv = encoder_block.downsample.conv
        og_padding = conv.padding
        conv.padding = (0, 1, 0, 1)
        save_padded_conv2d(conv, pathlib.Path(path, 'downsampler'))
        conv.padding = og_padding
