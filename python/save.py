...
def save_linear(linear, path):
    pathlib.Path(path).mkdir(parents=True, exist_ok=True)
    assert(linear.weight.dim() == 2)
    save_tensor(linear.weight.transpose(0, 1), 'weight', path) # PyTorch and Tinygrad strangely transpose linear weights so reverse that
...
def save_padded_conv2d(padded_conv2d, path):
    ...
    # Stride
    assert isinstance(padded_conv2d.stride, int) or (isinstance(padded_conv2d.stride, tuple) and len(set(padded_conv2d.stride)) == 1), "padded_conv2d.stride must be a scalar or a tuple with all elements equal"
    stride_value = padded_conv2d.stride if isinstance(padded_conv2d.stride, int) else padded_conv2d.stride[0]
    save_scalar(stride_value, 'stride', path)
